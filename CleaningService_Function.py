...
#The user will be prompted to enter the number of rooms in their house. House size will be assigned:
#a small house is <=5 rooms, medium is 6-10 rooms, and large is 11-15 rooms.
...
#Main program will call price function to determine the cost, using rooms and service as arguments
#price function will run an if conditional using the above described size definitions, to determine
#house size 
#Price function will run an if conditional on service with a nested conditional to select price
#dependent on the house size.
#Price function will return the size, opt, and cost.
#the main program will assign the variables size, opt, and cost respectively to the returned values
#and display the size and cleaning type selected the price.

# Developer: Meyer,Brianna   Date:02/06/2024
#------------------------------------------------------------
def price (rooms,service):
...
def main():
    ...
    #name, class, date
    print("Meyer, Brianna   02/06/2024")
...
better deal on FDA-complient ISO class-8-cleanroom deep-cleaning anywhere else!")
        ...
        #call function
        size,opt,cost= price(rooms,service)
